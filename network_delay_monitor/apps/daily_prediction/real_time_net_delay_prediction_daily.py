...
PAGE_TITLE = 'Cumulative Arrival Delay Prediction - Daily'
...
def render() -> html.Div:
    ...
    print_select_row = dbc.Row(
        [
            dbc.Col(html.Div(id='chosen-date-pred-daily')),
    ...
    return html.Div([
        ...
        dbc.Row(dcc.Markdown('', id='valid-inputs-daily'), style={'margin-top': '10px'}),
        dbc.Row(
            dbc.Col(dcc.Graph(id='graph-compare-daily')),
            style={'margin-top':'10px'},
            ),
        dbc.Row(dcc.Markdown('', id='daily-pred-info')),
        dbc.Row(dcc.Graph(id='data-table-daily')),
        dbc.Row(
            dbc.Col(dcc.Graph(id='graph-traj-daily')),
            style={'margin-top': '10px'},
        ),
    ])
...
def update_pred_figure(date, carrier=CARRIER, de_params=DE_PARAMS, model_name=MODEL_NAME, model_params=MODEL_PARAMS):
    ...
    model_pred_stats = _append_statement(model_pred_stats, f'- This model used a history of **{n_prev}** hours.')

    hr_curr = de_params['hr_curr']
    curr_time_string = (dt.datetime.combine(date_obj,dt.time(hour=hr_curr))).strftime('%b %d, %Y %H:%M')
    model_pred_stats = _append_statement(model_pred_stats, f'- This model assumes the current time is **{curr_time_string}**.')
    ...
    graph_info, node_data = prediction_utils.create_node_data_around_date(
        delay_df,
        date_obj,
        hr_shift,
    )
    ...
    if model_name == "GraphSAGE":
        ...
        model_input = torch.tensor(graph_info.iloc[24-hr_shift+hr_curr-n_prev:24-hr_shift+hr_curr].to_numpy(), dtype=torch.float32)
        # print("Model Input:\n", model_input)
        true_output = torch.tensor(graph_info.iloc[24-hr_shift+hr_curr+n_pred-1])
        # print("True Output:\n", true_output)

        pkl_data = PickleDataSet(filepath='./apps/daily_prediction/models/NAS-de_dataset-daily.pkl')
        model_data = pkl_data.load()
        edge_index = model_data['edge_index']
        edge_weight = model_data['edge_weight']
        
        model = GraphSAGE(
            in_channels=model_input.shape[0],
            out_channels=1,
            **model_params,
            )
        model.load_state_dict(torch.load('./apps/daily_prediction/models/simple-gnn_best_model.pt'))
        data_input = Data(x=model_input.permute(1,0), edge_index=edge_index, edge_weight=edge_weight[:,None])
        model_output = model.predict(data_input)
        # print("Model Output:\n", model_output)
        d = {}
        loc_list = list(NAS_30)
        loc_list.sort()
        d['icao'] = loc_list
        d['model_output'] = model_output.squeeze()
    ...
    window_data = graph_info.iloc[24-hr_shift+hr_curr-n_prev+1:24-hr_shift+hr_curr+1].reset_index()
    ...
    predict_data = graph_info.iloc[24-hr_shift+hr_curr+n_pred-1].to_frame().T
...
@app.callback(
    [
        Output('chosen-date-pred-daily', 'children'), 
        Output('graph-traj-daily', 'figure'), 
        Output('graph-compare-daily', 'figure'), 
        Output('valid-inputs-daily', 'children'),
        Output('daily-pred-info', 'children'),
        Output('data-table-daily', 'figure'),
    ], 
    [
        Input('pred-date-daily', 'date'), 
    ],
)
def update_daily_prediction_page(date):
    if date is not None:
        date_obj = dt.date.fromisoformat(date)
        date_str = date_obj.strftime('%b %d, %Y')
        fig, model_pred_stats, display_table, facet_fig = update_pred_figure(date)
